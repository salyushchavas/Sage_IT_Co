...
public class AgreementPdfService {
     ...
     * keeps it (DocumentStorageService — not the server's own disk,
     * which is wiped on every deploy) and records where (checklist 2.2).
     *
     * Throws on rendering failure — agreement-acceptance flow wraps the
     * call in try/catch so a PDF outage doesn't roll back the signature.
     */
    public byte[] renderSignedBytes(AgreementAcceptance row) {
        return renderPdf(row, true);
    }
    ...
    private byte[] renderBody(AgreementAcceptance row, TermsDocument doc, boolean signed) {
        try {
            ...
            document.add(new Paragraph(
                    signed && signedOnSite(row)
                            ? "This document was generated by " + doc.platform() + " and is the record "
                                    + "of the Participant's acceptance of this agreement, signed on the "
                                    + "website with their typed full legal name and signature, with the "
                                    + "date, IP address and browser recorded above. The fingerprint "
                                    + "identifies the exact text accepted. It is recorded as binding "
     ...
     */
    private PdfPTable buildAcceptanceRecordTable(
        ...
        if (row.getParticipantIdSnapshot() != null) {
            addRecordRow(table, "Participant ID", row.getParticipantIdSnapshot());
        }
        if (row.getProgramSnapshot() != null) {
            addRecordRow(table, "Program", row.getProgramSnapshot());
        }
        ...
        if (row.getTextSha256() != null) {
            addRecordRow(table, "Text fingerprint (SHA-256)", row.getTextSha256());
        }
        if (signed && signedOnSite(row)) {
            addRecordRow(table, "Signed with", "Typed legal name and "
                    + ("upload".equals(row.getSignatureMethod()) ? "uploaded" : "drawn")
                    + " signature, on the website");
        } else {
            // The older email-reply + one-time-code flow.
            ...
            addRecordRow(table, "Reply received",
                    signed
                            ? (row.getUserReplyReceivedAt() == null
                                    ? "—"
                                    : BusinessTime.stamp(row.getUserReplyReceivedAt(), businessZone)
    ...
    /** Signed on the website (no email reply / one-time code involved). */
    static boolean signedOnSite(AgreementAcceptance row) {
        return row.getAcceptanceCode() == null && row.getUserReplyReceivedAt() == null;
    }
