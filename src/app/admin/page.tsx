...
import {
  getAnalytics, getAdminCourses, publishCourse,
...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const [analyticsData, coursesData, requestsData] = await Promise.all([
          getAnalytics(),
          getAdminCourses(),
              ...
              {tabs.map((t) => (
                <button
                  ...
                  className={cn(
                    ...
                    tab === t.key
                      ? "bg-[#1B2A5C]/10 text-[#1B2A5C] border border-[#1B2A5C]/20"
                      : "text-zinc-600 hover:text-zinc-900 hover:bg-white/60"
                  )}
                >
                  {t.icon} {t.label}
                  {t.key === "requests" && requests.length > 0 && (
                    <span className="ml-auto text-xs bg-[#C87D5C] text-zinc-900 px-1.5 py-0.5 rounded-full">{requests.length}</span>
                  )}
                </button>
              ))}
            </nav>
          ...
          {tab === "overview" && analytics && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-10">
                {[
                  { label: "Total Users", value: analytics.totalUsers, icon: <Users className="w-5 h-5" />, color: "#1B2A5C" },
                  { label: "Total Courses", value: analytics.totalCourses, icon: <BookOpen className="w-5 h-5" />, color: "#C87D5C" },
                  { label: "Enrollments", value: analytics.totalEnrollments, icon: <GraduationCap className="w-5 h-5" />, color: "#0F1F44" },
                  { label: "Revenue", value: formatMoney(analytics.totalRevenue, "$0.00"), icon: <DollarSign className="w-5 h-5" />, color: "#10b981" },
                ].map((card) => (
                ...
                <div className="space-y-3">
                  {requests.map((req) => (
                    <div key={req.id} className="bg-white/60 backdrop-blur-xl border border-zinc-200 rounded-2xl p-4 flex items-center gap-4">
                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#C87D5C] to-[#1B2A5C] flex items-center justify-center text-zinc-900 font-bold text-sm shrink-0">
