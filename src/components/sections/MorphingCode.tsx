...
const codeSnippets = [
  {
    ...
    lang: "terraform",
    lines: [
      { text: 'resource "aws_lambda_function" "ai_engine" {', color: "#7c3aed" },
      { text: '  function_name = "sage-inference"', color: "#06b6d4" },
      { text: '  runtime       = "python3.12"', color: "#06b6d4" },
      { text: '  memory_size   = 2048', color: "#00d4ff" },
      { text: '  timeout       = 30', color: "#00d4ff" },
      { text: "", color: "#555" },
      { text: "  environment {", color: "#7c3aed" },
      { text: "    variables = {", color: "#a855f7" },
      { text: '      MODEL_ENDPOINT = var.sagemaker_ep', color: "#06b6d4" },
      { text: '      CACHE_TTL      = "3600"', color: "#06b6d4" },
      { text: "    }", color: "#a855f7" },
      { text: "  }", color: "#7c3aed" },
      { text: "}", color: "#7c3aed" },
    ],
  },
  {
    ...
    lang: "python",
    lines: [
      { text: "import torch", color: "#00d4ff" },
      { text: "from transformers import AutoModel", color: "#00d4ff" },
      { text: "", color: "#555" },
      { text: "class SagePredictor(nn.Module):", color: "#7c3aed" },
      { text: "    def __init__(self, config):", color: "#a855f7" },
      { text: "        super().__init__()", color: "#06b6d4" },
      { text: '        self.encoder = AutoModel.from_pretrained("sage-xl")', color: "#06b6d4" },
      { text: "        self.head = nn.Linear(768, config.num_classes)", color: "#06b6d4" },
      { text: "", color: "#555" },
      { text: "    def forward(self, x):", color: "#a855f7" },
      { text: "        embeddings = self.encoder(x).last_hidden_state", color: "#06b6d4" },
      { text: "        return self.head(embeddings[:, 0])", color: "#06b6d4" },
  ...
  },
  {
    ...
    lang: "kubernetes",
    lines: [
      { text: "apiVersion: apps/v1", color: "#00d4ff" },
      { text: "kind: Deployment", color: "#7c3aed" },
      { text: "metadata:", color: "#a855f7" },
      { text: "  name: sage-api-gateway", color: "#06b6d4" },
      { text: "  namespace: production", color: "#06b6d4" },
      { text: "spec:", color: "#a855f7" },
      { text: "  replicas: 12", color: "#00d4ff" },
      { text: "  strategy:", color: "#a855f7" },
      { text: "    type: RollingUpdate", color: "#06b6d4" },
      { text: "    maxSurge: 25%", color: "#06b6d4" },
      { text: "  template:", color: "#a855f7" },
      { text: "    spec:", color: "#a855f7" },
      { text: "      containers:", color: "#7c3aed" },
    ],
  },
  {
    label: "security.rs",
    lang: "rust",
    lines: [
      { text: "use sage_crypto::shield;", color: "#00d4ff" },
      { text: "use zero_trust::verify;", color: "#00d4ff" },
      { text: "", color: "#555" },
      { text: "#[derive(Shield)]", color: "#a855f7" },
      { text: "pub struct ThreatEngine {", color: "#7c3aed" },
      { text: "    model: NeuralDetector,", color: "#06b6d4" },
      { text: "    policy: ZeroTrustPolicy,", color: "#06b6d4" },
      { text: "}", color: "#7c3aed" },
      { text: "", color: "#555" },
      { text: "impl ThreatEngine {", color: "#7c3aed" },
      { text: "    pub async fn scan(&self, req: &Request)", color: "#a855f7" },
      { text: "        -> Result<Verdict, SecurityError> {", color: "#a855f7" },
      { text: "        let score = self.model.analyze(req).await?;", color: "#06b6d4" },
    ],
  },
  {
    label: "analytics.tsx",
    lang: "typescript",
    lines: [
      { text: 'import { Pipeline } from "@sage/data-nexus";', color: "#00d4ff" },
      { text: 'import { useRealtime } from "@sage/hooks";', color: "#00d4ff" },
      { text: "", color: "#555" },
      { text: "export function InsightDashboard() {", color: "#7c3aed" },
      { text: "  const { stream, metrics } = useRealtime({", color: "#a855f7" },
      { text: '    source: "kafka://events",', color: "#06b6d4" },
      { text: "    window: '5m',", color: "#06b6d4" },
      { text: "    aggregate: ['count', 'p99_latency'],", color: "#06b6d4" },
      { text: "  });", color: "#a855f7" },
      { text: "", color: "#555" },
      { text: "  return (", color: "#7c3aed" },
      { text: "    <Pipeline.Visualize data={stream} />", color: "#06b6d4" },
      { text: "  );", color: "#7c3aed" },
    ],
  },
];
