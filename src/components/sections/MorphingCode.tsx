...
const codeSnippets = [
  {
    ...
    lang: "terraform",
    lines: [
      { text: 'resource "aws_lambda_function" "ai_engine" {', color: "#E5B62E" },
      { text: '  function_name = "sage-inference"', color: "#14653F" },
      { text: '  runtime       = "python3.12"', color: "#14653F" },
      { text: '  memory_size   = 2048', color: "#22C55E" },
      { text: '  timeout       = 30', color: "#22C55E" },
      { text: "", color: "#555" },
      { text: "  environment {", color: "#E5B62E" },
      { text: "    variables = {", color: "#FBBF24" },
      { text: '      MODEL_ENDPOINT = var.sagemaker_ep', color: "#14653F" },
      { text: '      CACHE_TTL      = "3600"', color: "#14653F" },
      { text: "    }", color: "#FBBF24" },
      { text: "  }", color: "#E5B62E" },
      { text: "}", color: "#E5B62E" },
    ],
  },
  {
    ...
    lang: "python",
    lines: [
      { text: "import torch", color: "#22C55E" },
      { text: "from transformers import AutoModel", color: "#22C55E" },
      { text: "", color: "#555" },
      { text: "class SagePredictor(nn.Module):", color: "#E5B62E" },
      { text: "    def __init__(self, config):", color: "#FBBF24" },
      { text: "        super().__init__()", color: "#14653F" },
      { text: '        self.encoder = AutoModel.from_pretrained("sage-xl")', color: "#14653F" },
      { text: "        self.head = nn.Linear(768, config.num_classes)", color: "#14653F" },
      { text: "", color: "#555" },
      { text: "    def forward(self, x):", color: "#FBBF24" },
      { text: "        embeddings = self.encoder(x).last_hidden_state", color: "#14653F" },
      { text: "        return self.head(embeddings[:, 0])", color: "#14653F" },
  ...
  },
  {
    ...
    lang: "kubernetes",
    lines: [
      { text: "apiVersion: apps/v1", color: "#22C55E" },
      { text: "kind: Deployment", color: "#E5B62E" },
      { text: "metadata:", color: "#FBBF24" },
      { text: "  name: sage-api-gateway", color: "#14653F" },
      { text: "  namespace: production", color: "#14653F" },
      { text: "spec:", color: "#FBBF24" },
      { text: "  replicas: 12", color: "#22C55E" },
      { text: "  strategy:", color: "#FBBF24" },
      { text: "    type: RollingUpdate", color: "#14653F" },
      { text: "    maxSurge: 25%", color: "#14653F" },
      { text: "  template:", color: "#FBBF24" },
      { text: "    spec:", color: "#FBBF24" },
      { text: "      containers:", color: "#E5B62E" },
    ],
  },
  {
    label: "security.rs",
    lang: "rust",
    lines: [
      { text: "use sage_crypto::shield;", color: "#22C55E" },
      { text: "use zero_trust::verify;", color: "#22C55E" },
      { text: "", color: "#555" },
      { text: "#[derive(Shield)]", color: "#FBBF24" },
      { text: "pub struct ThreatEngine {", color: "#E5B62E" },
      { text: "    model: NeuralDetector,", color: "#14653F" },
      { text: "    policy: ZeroTrustPolicy,", color: "#14653F" },
      { text: "}", color: "#E5B62E" },
      { text: "", color: "#555" },
      { text: "impl ThreatEngine {", color: "#E5B62E" },
      { text: "    pub async fn scan(&self, req: &Request)", color: "#FBBF24" },
      { text: "        -> Result<Verdict, SecurityError> {", color: "#FBBF24" },
      { text: "        let score = self.model.analyze(req).await?;", color: "#14653F" },
    ],
  },
  {
    label: "analytics.tsx",
    lang: "typescript",
    lines: [
      { text: 'import { Pipeline } from "@sage/data-nexus";', color: "#22C55E" },
      { text: 'import { useRealtime } from "@sage/hooks";', color: "#22C55E" },
      { text: "", color: "#555" },
      { text: "export function InsightDashboard() {", color: "#E5B62E" },
      { text: "  const { stream, metrics } = useRealtime({", color: "#FBBF24" },
      { text: '    source: "kafka://events",', color: "#14653F" },
      { text: "    window: '5m',", color: "#14653F" },
      { text: "    aggregate: ['count', 'p99_latency'],", color: "#14653F" },
      { text: "  });", color: "#FBBF24" },
      { text: "", color: "#555" },
      { text: "  return (", color: "#E5B62E" },
      { text: "    <Pipeline.Visualize data={stream} />", color: "#14653F" },
      { text: "  );", color: "#E5B62E" },
    ],
  },
];
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none hidden lg:block" style={{ zIndex: 0 }}>
        ...
        <defs>
          <linearGradient id="lineGrad" x1="0%" y1="0%" x2="100%" y2="0%">
            ...
            <stop offset="100%" stopColor="#E5B62E" stopOpacity="0.5" />
          </linearGradient>
        </defs>
      </svg>
    </div>
